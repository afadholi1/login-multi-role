...
const Sidebar = ({ user, logout }) => {
  ...
  const menuItems = [
    { name: "Dashboard", path: "/dashboard", icon: <LayoutDashboard size={20} />, role: "all" },
    { name: "Admin Panel", path: "/admin", icon: <Users size={20} />, role: "admin" },
    { name: "Settings", path: "/settings", icon: <Settings size={20} />, role: "all" },
  ];
  ...
  return (
    <div className="fixed left-0 top-16 h-[calc(100vh-64px)] w-64 bg-white border-r border-slate-200 p-4 hidden md:flex flex-col">
      <div className="flex-1 space-y-2">
        {menuItems.map((item) => {
          // Hanya tampilkan menu Admin jika role-nya admin
